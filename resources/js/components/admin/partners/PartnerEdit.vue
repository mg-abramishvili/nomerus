<template>
    <div class="w-100">
        ...
        />
        <button @click="updatePartner(partner.id)" class="btn btn-danger">Сохранить</button>
    </div>
</template>
...
export default {
    ...
    created() {
        axios
            .get(`/api/admin/partner/${this.$route.params.id}`)
            .then(response => (
                this.partner = response.data,
                this.$parent.subheader = this.partner.name,
    ...
    methods: {
        updatePartner(id) {
            ...
            if(this.name.length && this.image) {
                axios
                .post(`/api/admin/partner/${id}`, { name: this.name, text: this.text, image: this.image })
                .then(response => (
